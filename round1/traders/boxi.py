...
class Logger:
    ...
    def flush(
        ...
        output = {
            ...
            "orders": [order.__dict__ for p in orders for order in orders[p]],
...
class Trader:
    ...
    def __init__(self) -> None:
        self.logger = Logger()
        self.product_osmium = "ASH_COATED_OSMIUM"
        self.product_pepper = "INTARIAN_PEPPER_ROOT"
        self.pos_limits: Dict[str, int] = {
            self.product_osmium: 50,
            self.product_pepper: 60,
        }
    ...
    def _fair_value(self, product: str, order_depth: OrderDepth) -> float:
        ...
        if best_bid is None or best_ask is None:
            return 10000.0 if product == self.product_osmium else self.ema_mid.get(product, 0.0)
        ...
        # Ash-coated osmium dostaje stałą bazę + tilt (jak dawniej EMERALDS)
        if product == self.product_osmium:
